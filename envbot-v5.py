...
# Add a StreamHandler to also output logs to the console
...
# Check if ADMIN_ID is not None before converting to int
if ADMIN_ID is not None:
    ADMIN_ID = int(ADMIN_ID)
else:
    print("Error: ADMIN_ID is not set in the environment variables.")
...
def button(update: Update, context: CallbackContext):
    ...
    if query.data == 'show_users':
        ...
        print("Show Users button clicked")
        ...
        print("Edit User Budget button clicked")
...
def edit_user_budget(update: Update, context: CallbackContext):
    ...
    if user_to_edit in allowed_users:
        ...
        print("User's budget updated successfully")
    else:
        update.message.reply_text("User not found.")
        print("User not found")
    show_button_menu(update, context)
...
def main():
    ...
    if ADMIN_ID is not None:
        updater.bot.send_message(chat_id=ADMIN_ID, text="Bot is running.")
